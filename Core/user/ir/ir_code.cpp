...
IR_CodeTable::IR_CodeTable(const char *controllerName, uint16_t customCode, const IR_CODE *pCodeTable, int tableLen)
{
	...
	if (m_nController < MAX_CONTROLLER)
		m_pController[m_nController++] = this;	/// 自動將定義的遙控器加入清單
}
 ...
 *
 */
bool	IR_CodeTable::SearchControllers(uint16_t customcode, uint8_t datacode, const char **ppControllerName, const char **ppKeyname)
{
