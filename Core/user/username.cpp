...
extern "C" void usermain() {
	...
	waitState =
			(HAL_GPIO_ReadPin(IRDA_GPIO_Port, IRDA_Pin) == GPIO_PIN_SET) ?
					MARK : SPACE;
	...
	for (int y = 0; y < 70; y += 10) {
		...
		ssd1306_WriteString("ABCDEFGHIJKLMNOPQR", Font_5x7, White);
	}
...
//		const char *pController = ir_parser.GetController();
...
//		ssd1306_printf(Font_5x7, White, "%03d %04X %-10s", ++nBtn, irCode,
//				pBtnName);
//
//		repeatX = 0;
//		repeatY = 16;
...
void hw2_uid() {
	...
	printf("LOT-WAF-X-Y = %c%c%c%c%c%c%c-%02X-%04X-%04X\r\n", pLot[7], pLot[6],
			pLot[5], pLot[4], pLot[3], pLot[2], pLot[1], *waf, *x, *y);
	...
	for (int nVector = 0; pVector < &g_pfnVectorsEnd; nVector++) {
		...
		else if ((*pVector & 0xFFFFFFFE)
				== ((uint32_t) &Default_Handler & 0xFFFFFFFE))
			printf("Vector %3d = %08lX (Default handler)\r\n", nVector,
					*pVector++);
		else if (nVector == 0)
			printf("Vector %3d = %08lX\r\n", nVector, *pVector++);
		else if ((*pVector & 0xFFFFFFFE)
				== ((uint32_t) &Reset_Handler & 0xFFFFFFFE))
